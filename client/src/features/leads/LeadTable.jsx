  ...
  <tr className="animate-pulse">
    {COLUMNS.map((col) => (
      <td key={col} className="px-6 py-4">
        <div className="h-4 bg-gray-100 rounded-full" style={{ width: `${Math.random() * 40 + 50}%` }} />
      </td>
    ))}
  </tr>
);
  ...
  <tr>
    <td colSpan={COLUMNS.length}>
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-gray-500">Failed to load leads. Please refresh the page.</p>
      </div>
    </td>
  </tr>
);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-100" role="grid">
          <thead>
      ...
      {!isLoading && !isError && leads.length > 0 && (
        <div className="px-6 py-3 border-t border-gray-100 bg-gray-50/40">
          <p className="text-xs text-gray-500">
            Showing <span className="font-medium text-gray-700">{leads.length}</span> lead{leads.length !== 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
