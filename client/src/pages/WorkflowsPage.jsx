...
const WorkflowsPage = () => {
  ...
  const { data: workflowsData, isLoading, isError, error, isFetching } = useGetWorkflowsQuery(
    undefined,
    {
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Workflows</h2>
      ...
      {/* Main content */}
      {isError && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-red-100 dark:border-red-900/50 shadow-sm p-6">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/50 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-slate-300 mt-1">{apiErrorMessage}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5 animate-pulse"
            >
              ...
              <div className="h-24 bg-gray-50 dark:bg-slate-900 rounded-xl mt-4" />
            </div>
          ))}
        </div>
      )}
      ...
      {isEmpty && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-12 flex flex-col items-center justify-center text-center">
          <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 rounded-2xl flex items-center justify-center mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-2">No workflows yet</h3>
          <p className="text-gray-500 dark:text-slate-300 max-w-sm text-sm">
